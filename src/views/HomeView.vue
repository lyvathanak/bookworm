  ...
  <div class="home-view">
    ...
    <div v-if="showSuccessMessage" class="success-message">
      <div class="success-content">
        <h3>✓ Added to Cart!</h3>
        <p>{{ lastAddedBook }} has been added to your cart.</p>
...
const getStarClass = (position, rating) => {
  ...
  if (ratingValue >= position) {
    return 'filled';
  } else if (ratingValue >= position - 0.5) {
    return 'half-filled';
  } else {
    return '';
  }
...
// Cart functionality using the store
const addToCart = (book) => {
  const success = cartStore.addToCart(book)
  if (success) {
    lastAddedBook.value = book.title
